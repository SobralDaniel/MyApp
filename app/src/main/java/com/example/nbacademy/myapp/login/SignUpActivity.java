...
public class SignUpActivity extends AppCompatActivity {
    ...
    public boolean validate() {
        ...
        if (password.isEmpty() || password.length() < 4 || password.length() > 10) {
            ...
            _passwordText.setError("passwords must match");
            valid = false;
        }
         else{
        ...
        if(name.isEmpty())
            _nameText.setError("must type name");
        else _nameText.setError(null);

        if(age.isEmpty())
            _ageText.setError("must type age");
        else _ageText.setError(null);

        if(nif.isEmpty())
            _nifText.setError("must type NIF");
        else _nifText.setError(null);

        if(number.isEmpty())
           _numberText.setError("must type number");
        else _numberText.setError(null);
